...
#     NAME => q[MyApp]
#     NO_META => q[1]
#     PREREQ_PM => { Moose=>q[0], ExtUtils::MakeMaker=>q[6.36], namespace::autoclean=>q[0], Catalyst::Runtime=>q[5.90112], Config::General=>q[0], Test::More=>q[0.88], Catalyst::Action::RenderView=>q[0], Catalyst::Plugin::Static::Simple=>q[0], Catalyst::Plugin::ConfigLoader=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.01]
...
pure_all :: $(INST_SCRIPT)/myapp_test.pl $(INST_SCRIPT)/myapp_server.pl $(INST_SCRIPT)/myapp_cgi.pl $(INST_SCRIPT)/myapp_create.pl $(INST_SCRIPT)/myapp_fastcgi.pl
	$(NOECHO) $(NOOP)
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>ubuntu,,,</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="i686-linux-gnu-thread-multi-64int-5.18" />' >> $(DISTNAME).ppd
